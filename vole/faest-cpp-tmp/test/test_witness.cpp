#include "test_witness.hpp"

// clang-format off

const std::array<uint8_t, 16> AES_ECB_128_KEY {
    0x42, 0x13, 0x4f, 0x71, 0x34, 0x89, 0x1b, 0x16, 0x82, 0xa8, 0xab, 0x56, 0x76, 0x27, 0x30, 0x0c
};

const std::array<uint8_t, 16> AES_ECB_128_INPUT {
    0x7b, 0x60, 0x66, 0xd6, 0x5a, 0x73, 0xd6, 0x00, 0xa0, 0xae, 0xf0, 0x1c, 0x8b, 0x19, 0x17, 0x40
};

const std::array<uint8_t, 16> AES_ECB_128_OUTPUT {
    0x89, 0x13, 0xaf, 0x07, 0x31, 0xb5, 0x81, 0xdf, 0x8a, 0x0a, 0xb5, 0x6e, 0x08, 0x3c, 0x6b, 0x7c
};

const std::array<uint8_t, 200> AES_ECB_128_EXTENDED_WITNESS {
    
};

const std::array<uint8_t, 24> AES_ECB_192_KEY {
    0x7f, 0x64, 0xa4, 0x6b, 0xbd, 0x02, 0x67, 0x2c, 0xed, 0x19, 0xfb, 0x73, 0x5b, 0xf0, 0x46, 0xaf,
    0x23, 0x6e, 0x38, 0x79, 0x85, 0x13, 0x79, 0xd3
};

const std::array<uint8_t, 32> AES_ECB_192_INPUT {
    0x2f, 0xae, 0x1b, 0x7c, 0x4a, 0x8f, 0xb6, 0x1c, 0x15, 0x7c, 0x4d, 0xe2, 0x9d, 0x35, 0x62, 0x33,
    0x63, 0x94, 0x75, 0x39, 0x50, 0x2d, 0x7e, 0xa5, 0xf7, 0x33, 0xd0, 0xca, 0x3c, 0xc2, 0xb5, 0xd0
};

const std::array<uint8_t, 32> AES_ECB_192_OUTPUT {
    0x1a, 0xb4, 0x2c, 0x3c, 0xde, 0xb6, 0xb5, 0x08, 0xff, 0xc8, 0x3d, 0x5b, 0x48, 0x9f, 0x62, 0xca,
    0xdd, 0x3f, 0x53, 0x92, 0xbb, 0x4b, 0x0a, 0xe3, 0xed, 0xf0, 0xe9, 0xe7, 0x0c, 0x4d, 0xb4, 0x2c
};

// NB: generated with faest-c
const std::array<uint8_t, 408> AES_ECB_192_EXTENDED_WITNESS {
    
};

const std::array<uint8_t, 32> AES_ECB_256_KEY {
    0xa9, 0x86, 0x63, 0xac, 0x8a, 0x05, 0x78, 0xbe, 0xd7, 0x2c, 0x80, 0x91, 0x07, 0x67, 0xce, 0x11,
    0xf1, 0x79, 0x59, 0xde, 0x6a, 0x99, 0xbb, 0xdc, 0x75, 0xb2, 0x04, 0x63, 0x6f, 0x1d, 0xd2, 0x5f
};

const std::array<uint8_t, 32> AES_ECB_256_INPUT {
    0x78, 0xff, 0xd6, 0xd1, 0x95, 0x73, 0xdb, 0x9f, 0xa4, 0x08, 0xe8, 0xcb, 0x29, 0xf8, 0x2e, 0x27,
    0x96, 0xe0, 0x8f, 0x0d, 0xf9, 0xf1, 0x8a, 0x1d, 0x67, 0xd5, 0xec, 0x22, 0x14, 0x92, 0x32, 0x17
};

const std::array<uint8_t, 32> AES_ECB_256_OUTPUT {
    0xef, 0x34, 0x98, 0xa1, 0x9e, 0xab, 0x0a, 0x9e, 0xad, 0x99, 0xd7, 0xf2, 0xe1, 0x68, 0xf5, 0xad,
    0x18, 0xdf, 0x02, 0x86, 0x6d, 0xf4, 0x2b, 0x80, 0x24, 0xe3, 0x9e, 0x24, 0xeb, 0x51, 0x83, 0x67
};

// NB: generated with faest-c
const std::array<uint8_t, 500> AES_ECB_256_EXTENDED_WITNESS {
    
};

const std::array<uint8_t, 16> RIJNDAEL_EM_128_KEY = AES_ECB_128_KEY;

const std::array<uint8_t, 16> RIJNDAEL_EM_128_INPUT = AES_ECB_128_INPUT;

const std::array<uint8_t, 16> RIJNDAEL_EM_128_OUTPUT {
    0xb8, 0xcf, 0x49, 0x82, 0x1b, 0x05, 0xe3, 0xfd, 0x4c, 0xb6, 0x61, 0x5c, 0x5f, 0x79, 0x77, 0x2f
};

const std::array<uint8_t, 160> RIJNDAEL_EM_128_EXTENDED_WITNESS {
    
};

const std::array<uint8_t, 24> RIJNDAEL_EM_192_KEY = {
    0x24, 0x18, 0x87, 0x72, 0xc5, 0x1f, 0xbe, 0x52, 0xc0, 0xcd, 0x0b, 0xed, 0xbe, 0x6a, 0x4c, 0x04,
    0xb3, 0x75, 0x89, 0x7d, 0x36, 0x9b, 0x7e, 0x62
};

const std::array<uint8_t, 24> RIJNDAEL_EM_192_INPUT {
    0xc1, 0xa3, 0xc0, 0x22, 0xe7, 0x18, 0x93, 0x5f, 0x46, 0x63, 0x03, 0x86, 0xaf, 0xa3, 0xd3, 0xf2,
    0xc0, 0x72, 0x0b, 0x10, 0xbf, 0x26, 0x6c, 0x19,
};

const std::array<uint8_t, 24> RIJNDAEL_EM_192_OUTPUT {
    0xbf, 0x71, 0x16, 0xfd, 0x88, 0xb0, 0x75, 0x2f, 0x06, 0x01, 0xc3, 0x14, 0xdc, 0xbb, 0xa6, 0x25,
    0x6b, 0x8e, 0xc4, 0x5b, 0xd2, 0x4e, 0x10, 0x84
};

const std::array<uint8_t, 288> RIJNDAEL_EM_192_EXTENDED_WITNESS {
    
};

const std::array<uint8_t, 32> RIJNDAEL_EM_256_KEY = {
    0xc0, 0xcd, 0x0b, 0xed, 0xbe, 0x6a, 0x4c, 0x04, 0xb3, 0x75, 0x89, 0x7d, 0x36, 0x9b, 0x7e, 0x62,
    0xaa, 0x6a, 0x6f, 0x17, 0x13, 0xd2, 0x7a, 0x71, 0xfe, 0x98, 0x9e, 0x93, 0xdc, 0x79, 0xd2, 0x7d
};

const std::array<uint8_t, 32> RIJNDAEL_EM_256_INPUT = {
    0xc1, 0xa3, 0xc0, 0x22, 0xe7, 0x18, 0x93, 0x5f, 0x46, 0x63, 0x03, 0x86, 0xaf, 0xa3, 0xd3, 0xf2,
    0xc0, 0x72, 0x0b, 0x10, 0xbf, 0x26, 0x6c, 0x19, 0x24, 0x18, 0x87, 0x72, 0xc5, 0x1f, 0xbe, 0x52,
};

const std::array<uint8_t, 32> RIJNDAEL_EM_256_OUTPUT {
    0xf4, 0x1b, 0x6b, 0xb2, 0x22, 0x5c, 0xaf, 0xfc, 0x82, 0x31, 0xcc, 0xe0, 0x21, 0x9e, 0x2c, 0xcc,
    0xb0, 0xf8, 0x0e, 0x68, 0x0d, 0xf2, 0xdf, 0xef, 0xca, 0x1b, 0x96, 0x57, 0x18, 0x42, 0x00, 0x25
};

const std::array<uint8_t, 448> RIJNDAEL_EM_256_EXTENDED_WITNESS {
    
};

// clang-format on
