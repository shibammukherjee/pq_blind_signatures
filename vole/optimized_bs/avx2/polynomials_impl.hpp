...
template <> struct poly<128>
{
    ...
    // --- START ---
    inline static poly128 from_4_byte(uint8_t byte, bool msb_4_bits)
    {
        poly1 bits[4];
        size_t bit_offset = 0;
        if (msb_4_bits) {
...
template <> struct poly<192>
{
    ...
    // --- START ---
    inline static poly192 from_4_byte(uint8_t byte, bool msb_4_bits)
    {
        poly1 bits[4];
        size_t bit_offset = 0;
        if (msb_4_bits) {
...
template <> struct poly<256>
{
    ...
    // --- START ---
    inline static poly256 from_4_byte(uint8_t byte, bool msb_4_bits)
    {
        poly1 bits[4];
        size_t bit_offset = 0;
        if (msb_4_bits) {
...
// --- START ---
inline poly128 poly128::from_4_poly1(const poly1* bits)
{
    poly128 out = from_1(bits[0]);
...
// --- START ---
inline poly192 poly192::from_4_poly1(const poly1* bits)
{
    poly192 out = from_1(bits[0]);
    for (size_t i = 1; i < 4; ++i)
...
// --- START ---
inline poly256 poly256::from_4_poly1(const poly1* bits)
{
    poly256 out = from_1(bits[0]);
    for (size_t i = 1; i < 4; ++i)
...
// --- START ---
inline poly128 poly128::from_4_self(const poly128* polys)
{
    poly256 out = poly256::from<128>(polys[0]);
    for (size_t i = 1; i < 4; ++i)
...
// --- START ---
inline poly192 poly192::from_4_self(const poly192* polys)
{
    poly384 out = poly384::from<192>(polys[0]);
    for (size_t i = 1; i < 4; ++i)
...
// --- START ---
inline poly256 poly256::from_4_self(const poly256* polys)
{
    poly512 out = poly512::from<256>(polys[0]);
    for (size_t i = 1; i < 4; ++i)
