# # Step 1: Define the field GF(2^4)
...
x = R.gen()
F = GF(2**512, name='a', modulus=x^512 + x^8 + x^5 + x^2 + 1)  # Choose any irreducible poly
a = F.gen()

# Step 2: Convert field element to 512-bit vector
def element_to_vector(e):
    coeffs = e.polynomial().coefficients(sparse=False)
    return vector(GF(2), coeffs + [0]*(512 - len(coeffs)))  # pad to 512

# Step 3: Build transformation matrix from squaring basis elements
basis = [a^i for i in range(512)]  # [1, a, a^2, a^3, ....]
columns = [element_to_vector(b^2) for b in basis]
T = Matrix(GF(2), 512, 512, columns).transpose()
...
for i in range(512):
    terms = [f"x[{j}]" for j in range(512) if T[i,j] == 1]
    if terms:
        rhs = " + ".join(terms)
    else:
        rhs = "0"
